...
local function getClockText()
    -- Der Server laeuft zwei Stunden hinter der Schweizer Zeit.
    -- Deshalb werden dauerhaft zwei Stunden addiert.
    local TIME_OFFSET_SECONDS = 2 * 60 * 60
    ...
    -- Ersatzanzeige mit ebenfalls zwei Stunden Zeitversatz.
    local fallbackTime =
        (os.time() + 2) % 24

    return textutils.formatTime(fallbackTime, true)
end
